...
func TestClassifyMonthlyExhaustionFlagIsExactOnly(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "exact you've reached", events: []protocol.Event{{Type: "run_finished", Data: map[string]any{
			"status": "failed",
			"error":  "Error code: 403 - insufficient_quota: You've reached your usage limit for this billing cycle. Please try again later or upgrade your plan.",
		}}}},
		{name: "exact you have reached", events: []protocol.Event{{Type: "run_finished", Data: map[string]any{
			"status": "failed",
			"error":  "You have reached your usage limit for this billing cycle.",
		}}}},
		{name: "exact usage limit phrase", events: []protocol.Event{{Type: "run_finished", Data: map[string]any{
			"status": "failed",
			"error":  "usage limit for this billing cycle exceeded",
		}}}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			got := ClassifyAttempt(tc.events, now)
