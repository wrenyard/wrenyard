...
func TestProviderModelMap(t *testing.T) {
	...
	if _, ok := r.LookupProviderModel("codebuddy", "kimi-k2.6"); ok {
		t.Fatal("codebuddy should no longer own retired Kimi K2.6")
	}
...
func TestCursorProviderBinding(t *testing.T) {
	...
	for _, model := range []string{"composer-2.5", "grok-4.6", "kimi-k3", "claude-opus-5"} {
		if err := b.ValidateModel(model); err != nil {
