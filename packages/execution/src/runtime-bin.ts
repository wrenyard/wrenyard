...
export function resolveRuntimeBin(
  ...
  const suiteRoot = options.suiteRoot ?? discoverSuiteRoot(env)
  if (suiteRoot) {
    const candidates = [
...
function discoverSuiteRoot(env: NodeJS.ProcessEnv): string | undefined {
  const explicit = env.WRENYARD_ROOT?.trim();
  let current = explicit ? resolve(explicit) : dirname(fileURLToPath(import.meta.url));
  while (true) {
    if (existsSync(join(current, 'pnpm-workspace.yaml')) && existsSync(join(current, 'release-manifest.json'))) return current;
