...
export const quota = {
  async read(source: QuotaSource) {
    const raw = await source.read();
    try { return normalizeClaudeQuota(raw); }
    catch (error) {
      if (object(raw).source !== 'claude-sources') throw error;
      return normalizeClaudeQuota(await source.read('fallback'));
    }
  },
  bindings: [],
...
function normalizeClaudeQuota(raw: unknown) {
