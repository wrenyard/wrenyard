...
\`\`\`
`], labels: ["implementation_plan","functional_unit_set","运行时填入任务输入"] } as const
...
const definition = {
  ...
  config: {
    ...
    prompt: withTaskPromptTemplates(({ implementation_plan, functional_unit_set = undefined, context = '' }: z.infer<typeof InputSchema>) => renderTaskPromptTemplate(TASK_PROMPT_TEMPLATE_1, [JSON.stringify(implementation_plan, null, 2),
functional_unit_set ? JSON.stringify(functional_unit_set, null, 2) : '(not provided)',
context ? `## Context\n${context}` : '']), [TASK_PROMPT_TEMPLATE_1]),
  },
