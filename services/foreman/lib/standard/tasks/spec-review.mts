...
\`\`\`
`], labels: ["specDocument","spec_content","full_context","feature_point_set","functional_unit_set"] } as const
...
const definition = {
  ...
  config: {
    ...
    prompt: withTaskPromptTemplates(({ spec_content, full_context, feature_point_set = undefined, functional_unit_set = undefined }: z.infer<typeof InputSchema>) => renderTaskPromptTemplate(TASK_PROMPT_TEMPLATE_1, [specDocument,
spec_content,
full_context,
feature_point_set ? JSON.stringify(feature_point_set, null, 2) : '(none provided)',
functional_unit_set ? JSON.stringify(functional_unit_set, null, 2) : '(none provided)']), [TASK_PROMPT_TEMPLATE_1]),
  },
