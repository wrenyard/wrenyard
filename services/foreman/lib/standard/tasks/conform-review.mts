...
\`\`\`
`], labels: ["architecture_direction","diff","运行时填入任务输入"] } as const
...
const definition = {
  ...
  config: {
    ...
    prompt: withTaskPromptTemplates(({
      ...
      diff,
      files_context = '',
    }: z.infer<typeof inputSchema>) => renderTaskPromptTemplate(TASK_PROMPT_TEMPLATE_1, [architecture_direction,
diff,
files_context ? `## Additional File Context\n${files_context}` : '']), [TASK_PROMPT_TEMPLATE_1]),
  },
