...
export class EntityManager {
  ...
  setHouseVisible(visible: boolean): void {
    ...
    if (this.house && !this.house.isDestroyed()) {
      if (visible && this.visible) this.house.showInactive();
  ...
  setWorkersVisible(visible: boolean): void {
    ...
    for (const worker of this.workers.values()) {
      ...
      if (visible && this.visible) worker.window.showInactive();
  ...
  /**
   * Overall Pet visibility, distinct from per-entity toggles. Hiding keeps
   * every window alive so re-showing resumes from the current snapshot without
   * a rebuild; only a real dispose destroys windows. When shown, each window is
   * re-shown only if its own entity type is enabled.
   */
  setVisible(visible: boolean): void {
    this.visible = visible;
    if (this.house && !this.house.isDestroyed()) {
      if (visible && this.config.entities.house) this.house.showInactive();
      else this.house.hide();
    }
    for (const worker of this.workers.values()) {
      if (worker.window.isDestroyed()) continue;
      if (visible && this.config.entities.workers) worker.window.showInactive();
      else worker.window.hide();
    }
  }

  isVisible(): boolean {
    if (this.house && !this.house.isDestroyed() && this.house.isVisible()) return true;
    for (const worker of this.workers.values()) {
      if (!worker.window.isDestroyed() && worker.window.isVisible()) return true;
    }
    return false;
  }
  ...
  private startFade(worker: WorkerEntity, to: number, destroyOnComplete: boolean): void {
    ...
    if (to > 0 && this.visible && this.config.entities.workers) {
