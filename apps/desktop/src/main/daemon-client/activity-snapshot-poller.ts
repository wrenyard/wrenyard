...
export type ActivitySnapshotRequest = (method: string, params?: unknown, options?: DaemonRequestOptions) => Promise<unknown>;

export interface ActivitySnapshotPollerOptions {
  ipcPath?: string;
  intervalMs?: number;
  request?: ActivitySnapshotRequest;
  /** Shared Desktop daemon client; used when no explicit `request` is given. */
  client?: DaemonClient;
...
export class ActivitySnapshotPoller {
  ...
  private readonly ownedClient: DaemonClient | undefined;
  ...
  constructor(opts: ActivitySnapshotPollerOptions) {
    this.ipcPath = opts.ipcPath ?? resolveDaemonIpcPath();
    this.intervalMs = opts.intervalMs ?? DEFAULT_POLL_INTERVAL_MS;
    if (opts.request) {
      this.request = opts.request;
    } else {
      this.ownedClient = opts.client ?? new WrenyardDaemonClient({ path: this.ipcPath });
      this.request = (method, params, options?) => this.ownedClient!.request(method, params, options);
    }
