...
import { PixelBuilder } from '../../src/pet/render';
import type { RenderTextStyle, ShapeCommand } from '../../src/pet/render/types';
...
describe('worker fixture contract', () => {
  it('has exact semantic fixture order and common base state', async () => {
    const { WORKER_FIXTURES, FIXTURE_VIEWPORT } = await import('../../tools/pet/scripts/preview/fixtures.mjs');
  ...
  it('keeps exact tool and mixed CJK bubble fixture semantics', async () => {
    const { WORKER_FIXTURES } = await import('../../tools/pet/scripts/preview/fixtures.mjs');
...
describe('worker label entity', () => {
  it('formats age and resolves hover task labels through production helpers', async () => {
    const { formatWorkerAge, resolveWorkerLabelText } = await import('../../src/pet/features/worker/scene/label');
  ...
  it('renders task background, border, padding and ellipsis with entity-owned nodes', async () => {
    const { createWorkerLabel, updateWorkerLabel } = await import('../../src/pet/features/worker/scene/label');
...
describe('worker bubble entity', () => {
  it('uses legacy CJK-dominant wrapping, ellipsis and measured widths', async () => {
    const {
      ...
      MAX_BUBBLE_WIDTH,
      WORKER_MAX_LINES,
    } = await import('../../src/pet/features/worker/scene/bubble');
    const calls: string[] = [];
  ...
  it('keeps full-text layout stable while reveal text changes', async () => {
    const { createWorkerBubble, updateWorkerBubble, revealCharCount } = await import('../../src/pet/features/worker/scene/bubble');
  ...
  it('uses rounded body commands, measured caret placement and linear fade', async () => {
    const { createWorkerBubble, updateWorkerBubble, bubbleAlpha } = await import('../../src/pet/features/worker/scene/bubble');
...
describe('worker tool cue entity', () => {
  it('uses timing alpha, exact count text and measured middle baseline conversion', async () => {
    const { createToolCue, updateToolCue, toolCueVisible, toolFlashAlpha } = await import('../../src/pet/features/worker/scene/tool-cue');
  ...
  it('ports tool palette and hollow center geometry exactly', async () => {
    const { createToolCue, updateToolCue } = await import('../../src/pet/features/worker/scene/tool-cue');
    const { darkenColor, lightenColor } = await import('../../src/pet/features/worker/scene/palette');
    const { surface, graphics } = mockSurface();
    const container = surface.createContainer();
    const node = createToolCue(container, surface);
...
describe('worker skin, hit region and badge helpers', () => {
  it('uses exactly the shared four phase values and no dozing branch', async () => {
    const { moodForPhase } = await import('../../src/pet/features/worker/scene/skin-drawer');
    ...
    const source = fs.readFileSync(path.join(rootDir, 'src/pet/features/worker/scene/skin-drawer.ts'), 'utf8');
    expect(source).not.toMatch(/phase\s*={2,3}\s*['"]dozing['"]/);
  });

  it('computes inclusive 40x32 scaled hit regions and passthrough', async () => {
    const { computeHitRegion, hitTest, isHovering, isPassthrough } = await import('../../src/pet/features/worker/scene/hit-regions');
  ...
  it('keeps unknown badge empty and known badges inside 10x10 ROI', async () => {
    const { drawClientBadge } = await import('../../src/pet/features/worker/scene/badge');
...
describe('worker activityPulseOffset fallback (FU-002 production helper)', () => {
  it('falls back to lastToolTs when activity is absent', async () => {
    const { activityPulseOffset } = await import('../../src/pet/features/worker/scene/timing');
