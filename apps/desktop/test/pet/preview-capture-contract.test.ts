...
describe('stdio guard contract', () => {
  it('isBrokenPipe matches only EPIPE code', async () => {
    const { isBrokenPipe } = await import('../../tools/pet/scripts/preview/stdio-guard.mjs');
  ...
  it('installBrokenPipeGuard suppresses EPIPE and rethrows other errors on an evented stream', async () => {
    const { installBrokenPipeGuard, isBrokenPipe } = await import('../../tools/pet/scripts/preview/stdio-guard.mjs');
...
describe('final preview capture contract', () => {
  it('keeps the exact 34-case fixture order', async () => {
    const { PREVIEW_FIXTURES } = await import('../../tools/pet/scripts/preview/fixtures.mjs');
  ...
  it('serializes all nine failure reasons as one failed JSON object without success fields', async () => {
    const { FAILURE_REASONS, parseInjectFailure, serializeFailure } = await import('../../tools/pet/scripts/preview/capture-contract.mjs');
  ...
  it('builds stable success manifest shape, order, thresholds, and paths', async () => {
    const fixtures = await import('../../tools/pet/scripts/preview/fixtures.mjs');
    const contract = await import('../../tools/pet/scripts/preview/capture-contract.mjs');
  ...
  it('pins houseSkin for all 9 house fixtures', async () => {
    const { HOUSE_FIXTURES } = await import('../../tools/pet/scripts/preview/fixtures.mjs');
  ...
  it('keeps package preview capture as the production harness command', () => {
    ...
    expect(pkg.scripts['pet:preview:capture']).toBe('electron --force-device-scale-factor=1 --force-color-profile=srgb tools/pet/scripts/preview-capture.mjs');
  });
