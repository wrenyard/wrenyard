...
const scannerPath = path.join(repoRoot, 'tools', 'pet', 'renderer-cleanup-scan.mjs');
...
describe('renderer cleanup scanner', () => {
  ...
  it('passes a pixi.js import inside src/pet/render/pixi', () => {
    const root = makeFixture();
    writeFixture(root, 'src/pet/render/pixi/inside.ts', "import { Application } from 'pixi.js';\n");
  ...
  it('default invocation scans tsconfig.json and fails on an old renderer path', () => {
    const root = makeFixture();
    writeFixture(root, 'tsconfig.json', '{\n  "include": ["src/renderer/**/*"]\n}\n');

    const payload = expectFailureResult(runDefaultScanner(root), 'src/renderer');

    expect(payload.file).toBe('tsconfig.json');
  });
