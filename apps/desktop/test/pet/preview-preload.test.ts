...
const preload = require('../../tools/pet/scripts/preview-capture-preload.cjs');

describe('preview capture preload contract', () => {
  it('reads exactly one serialized fixture argument', async () => {
    const { WORKER_FIXTURES, serializeFixture } = await import('../../tools/pet/scripts/preview/fixtures.mjs');
  ...
  it('synchronously publishes complete worker and house renderer state for matching subscriptions', async () => {
    const { WORKER_FIXTURES, HOUSE_FIXTURES } = await import('../../tools/pet/scripts/preview/fixtures.mjs');
