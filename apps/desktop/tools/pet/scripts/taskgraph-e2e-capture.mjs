...
// Uses the production TaskGraphWindowOwner bundle, production preloads and
// genuine Electron input. Fakes only the daemon request boundary so every IPC
// path (entity:open-self, slip:open-transcript, slip:close,
// entity:set-mouse-passthrough, transcript:retry, panel:close) runs through
// production handlers.
...
// standalone bundle of the production sources instead of loading a Pet-app
// dist artifact that no longer exists.
...
var NORMALIZED_SCRIPTS_DIR = normalizeTokenPath(path.join(rootDir, 'tools', 'pet', 'scripts'));
