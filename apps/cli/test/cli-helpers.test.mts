...
import { parsePositiveIntegerFlag, requireNoPositionals, requireSinglePositional } from '../src/helpers.mts'
import {
  errorMessage,
} from '../src/shared.mts'
import { readFileSync } from 'node:fs'
import { INVALID_PARAMS, ProtocolError } from '@wrenyard/daemon/protocol/errors'
...
test('CLI task run waits for the terminal result without status polling or follow-up hints', () => {
  const taskSource = readFileSync(new URL('../src/commands/task.mts', import.meta.url), 'utf8')
  const sharedSource = readFileSync(new URL('../src/shared.mts', import.meta.url), 'utf8')
