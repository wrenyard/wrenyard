...
import { buildDaemonInvocation, resolveDaemonSupervisorPaths } from '../src/daemon-supervisor.mts'
import type { DaemonLifecycleOptions } from '../src/daemon-supervisor.mts'
...
describe('buildDaemonInvocation', () => {
  it('uses process.execPath with on-disk tsx preflight/loader and the daemon entrypoint', () => {
    ...
    assert.ok(
      entrypoint.endsWith(join('apps', 'daemon', 'bin', 'daemon.mts')),
      `entrypoint must end with apps/daemon/bin/daemon.mts, got: ${entrypoint}`,
    )
  })

  it('does not construct tsx paths from a hardcoded apps/daemon/node_modules/tsx literal', () => {
    const inv = buildDaemonInvocation(makeOptions())
    const hardcodedLocalTsx = join('apps', 'daemon', 'node_modules', 'tsx')
