...
} from '../src/commands/update.mts'
import type {
  PlannedRestartPlan,
  PlannedRestartPlanSummary,
} from '@wrenyard/daemon/daemon/planned-restart-store'
import { foremanDir, suiteDir, type ForemanStatus } from '../src/shared.mts'
import { ForemanUpdateGit } from '@wrenyard/daemon/core/project/foreman-update'
import { closeDb } from '@wrenyard/daemon/db/connection'
...
test('default update checkout targets the Wrenyard suite root, not the foreman package', () => {
  ...
  assert.ok(foremanDir.endsWith(join('apps', 'daemon')), 'foremanDir is the daemon package root')
