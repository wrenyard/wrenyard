...
test('daemon bin directory only keeps the daemon entrypoint', () => {
  const daemonRoot = join(dirname(fileURLToPath(import.meta.url)), '..', '..', 'daemon')
  const entries = readdirSync(join(daemonRoot, 'bin')).sort()

  assert.deepEqual(entries, ['daemon.mts'])
})
...
test('daemon supervisor starts Node directly with the tsx loader and hides Windows subprocess windows', () => {
  const repoRoot = join(dirname(fileURLToPath(import.meta.url)), '..')
  const source = readFileSync(join(repoRoot, 'src', 'daemon-supervisor.mts'), 'utf-8')
  ...
  assert.match(source, /daemon\.mts/u)
