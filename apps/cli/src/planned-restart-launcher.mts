...
import { foremanDir, cliDir } from './shared.mts'
import { foremanStateRoot } from '@wrenyard/daemon/config/state'
import { errorMessage } from './shared.mts'
import type { PlannedRestartKind } from '@wrenyard/daemon/daemon/planned-restart-store'
...
export interface PlannedRestartLauncherDispatch {
  status: () => import('@wrenyard/daemon/daemon/dispatch-control').DispatchStatus
  updatePlannedRestart: (operationId: string, update: import('@wrenyard/daemon/daemon/planned-restart-store').PlannedRestartPlanUpdate) => void
  completePlannedRestart: (operationId: string) => void
  abortPlannedRestart: (operationId: string) => void
  failPlannedRestart: (operationId: string, failure: import('@wrenyard/daemon/daemon/planned-restart-store').PlannedRestartFailure) => void
  beginPlannedRestart: (plan: import('@wrenyard/daemon/daemon/planned-restart-store').PlannedRestartPlan) => void
}
...
export function buildPlannedRestartCoordinatorInvocation(options: {
  ...
  const args: string[] = [
    ...
    join(cliDir, 'src', 'planned-restart-coordinator-process.mts'),
    '--operation-id',
    options.operationId,
    '--kind',
    options.kind,
