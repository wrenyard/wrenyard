 ...
 * planned restart. It drives the actual `apps/cli/src/index.mts` CLI: it starts an
...
test('task-context planned restart drains, hands off to a new daemon, and keeps task output readable', {
  ...
  const binary = join(cliRoot, 'src', 'index.mts')
 ...
 * runs the real `apps/cli/src/index.mts daemon restart` (which detects task context
...
function writeFakeForgeScript(scriptPath: string): void {
  const lines = [
    ...
    '}',
    '',
    "const restart = spawnSync(process.execPath, [tsxCli, binary, 'daemon', 'restart', '--config', configPath, '--json'], { encoding: 'utf-8', env: process.env })",
    '',
    'let scheduled',
    'try {',
