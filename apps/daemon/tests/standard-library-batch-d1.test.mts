...
describe('standard/tasks edit — definition shape & config', () => {
  ...
  it('prompt preserves exact-path-only editing semantics', async () => {
    ...
    assert.match(prompt, /read the relevant target portions before editing/)
    assert.match(prompt, /bounded searches within the declared targets/)
    assert.match(prompt, /independent test task/)
    assert.match(prompt, /report the missing input rather than guessing/)
    assert.doesNotMatch(prompt, /must not outnumber Edit\/Write mutations/)
  })
})
