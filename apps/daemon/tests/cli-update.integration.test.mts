 ...
 *
 * It drives the real `apps/cli/src/index.mts` update command from a task whose fake
...
test('task-context update fast-forwards the temporary checkout, hands off to a new daemon, and keeps task output readable', {
  ...
  // The suite checkout is the git top-level that contains both the CLI and the
  // daemon; the update flow prepares the whole suite, not a single package.
  ...
  const binary = join(checkoutDir, 'apps', 'cli', 'src', 'index.mts')
  ...
  assert.ok(
    existsSync(join(seedDir, 'apps', 'cli', 'src', 'commands', 'update.mts')),
    'seed must contain the current update implementation under test',
  )
  assert.ok(
    existsSync(join(seedDir, 'apps', 'daemon', 'tests', 'cli-update.integration.test.mts')),
 ...
 * runs the real `apps/cli/src/index.mts update` (which detects task context via the
...
function writeFakeForgeScript(scriptPath: string): void {
  const lines = [
    ...
    "const tsxCli = process.env.FOREMAN_FAKE_FORGE_TSX",
    ...
    '}',
    '',
    "const update = spawnSync(process.execPath, [tsxCli, binary, 'update', '--config', configPath, '--json'], { encoding: 'utf-8', env: process.env })",
    '',
    'let scheduled',
    'try {',
