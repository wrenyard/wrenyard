...
/** Content width: percentage column plus an 8px trailing safety inset. */
...
/** All secondary labels share one left-aligned child-column baseline. */
...
/** Visible percentage text stays compact; remaining semantics come from the quota view. */
export function formatQuotaMenuPercentageText(remainingPct: number): string {
  return `${floorQuotaPercentage(remainingPct)}%`;
}
...
function renderMenuLine(
  ...
  if (row.error) {
    if (isFirstLine) {
      drawString(buffer, strideWidth, PROVIDER_X, lineTop, row.provider, PROVIDER_MAX, PROVIDER_COLOR);
    }
    drawString(buffer, strideWidth, QUOTA_MENU_CHILD_X, lineTop, row.error, ERROR_MAX, CHILD_COLOR);
    return;
  }

  // Monetary balance rows: provider | bal | amount starting at the bar column.
  // No track/fill/pace marker/percentage semantics.
  ...
  if (row.remainingPct !== null) {
    ...
    drawString(
      ...
      PCT_X,
      lineTop,
      formatQuotaMenuPercentageText(row.remainingPct),
      PCT_MAX,
      AMOUNT_COLOR,
    );
  }
...
function drawString(
  ...
  for (const raw of text.toLowerCase()) {
    ...
    drawGlyph(buffer, strideWidth, cursor, y, glyph, color);
    cursor += 6;
  }
}
