...
describe('overlay entry contracts', () => {
  ...
  it('builds production renderer bundles from overlay entries and copies overlay HTML via dedicated copy script', () => {
    ...
    expect(copyScript).not.toContain("panels', 'stats");
    expect(copyScript).not.toContain("panels', 'settings");
    expect(copyScript).toContain("for (const retired of ['settings.html', 'settings.js', 'stats.html', 'stats.js', 'panel.css'])");
